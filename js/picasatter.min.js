...
function fetch_main_window(id, src, summary, team){

    window.picasatter['team'] = team;
    $('#tw'+id+'new').html("");
    $("#main_window, .subwindow").fadeOut(200, function(){
        if($(this).attr('id')=='main_window')$(this).attr("src", src);
        else $(this).attr("src", "./images/" + team + ".jpg");
        $(this).fadeIn(200);
      }
    );
...
function countup(team){
    var now = $('#dcount' + team).html();
    if(isNaN(now))return false;
    now++;
    $('#dcount' + window.picasatter['team']).html(""+now);
}
...
function highlight_hide(elem, num){
    ...
    setTimeout(function(){
        ...
        elem.fadeOut('slow').fadeIn('fast', function(){
            ...
            if(check) setTimeout(function(){ countup(window.picasatter['team'])}, 1000);
          }
        );
       },1000
    );
...
function table_bind(elem, prefix, rows, columns, fn){
    ...
    for(i = 0; i < rows.length; i++){
        for(j = 0; j < columns.length; j++){
            ...
            $('#'+cid).bind("click", function(){fn(columns[j]);});
        }
    }
}
...
function draw_table(elem, rows, columns, value, fn){
    ...
    for(i = 0; i < rows.length; i++){
        ...
        for(j = 0; j < columns.length; j++){
            ...
            else row += '<td id="d' + rows[i] + columns[j] + '">' + value + '</td>';
        }
        row += '</tr>';
        elem.append(row);
    }
    ...
    if(fn){
        table_bind(elem, 'h', rows, columns, countup);  //header
        table_bind(elem, 'd', rows, columns, countdown);  //body
    }
    elem.append('</tbody>');
    return false;
}
    ...
    $.getJSON(url,function(json){
        ...
        $(json.feed.entry).each(function(){
	        ...
	        var thedatestr = thedate.getHours()+':'+thedate.getMinutes();
                ...
                elem.prepend(divstr);
                $('#tw'+id, elem).hide();
                $('#tw'+id, elem).bind("click", function(){fetch_main_window(id, src, summary, team);});
                $('#tw'+id+' img',elem).hide();
...
$(document).ready(function(){
    ...
    $('.render').each(function(e){
        $(this).bind("click", function(){toggle($(this), 'feed_check', "<div class=\"start\"></div>", "<div class=\"stop\"></div>");});
      }
    );
