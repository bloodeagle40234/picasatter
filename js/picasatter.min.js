...
function toggle(elem, cheker, tmsg, fmsg){
    if(window.picasatter[cheker]){
        window.picasatter[cheker] = false;
        $(elem).html(fmsg);
    }else{
        window.picasatter[cheker] = true;
        $(elem).html(tmsg);
    }
}
...
function draw_table(elem, rows, columns){
    elem = $(elem);
    ...
    for(i = 0; i < rows.length; i++){
        ...
        for(j = 0; j < columns.length; j++){
            ...
            else row += '<td id="d' + rows[i] + columns[j] + '">o</td>';
        }
        row += '</tr>'
        elem.append(row);
    }
    ...
    $.getJSON(url,function(json){
        ...
        $(json.feed.entry).each(function(){
	        ...
	        var thedatestr = thedate.getHours()+':'+thedate.getMinutes();
                ...
                //var thedatestr=thedate.getHours()+':'+thedate.getMinutes();
                var divstr = '<div id="tw'+this.gphoto$id.$t+'" class="thumbnail"><img src="'+src+'" ></div>';
